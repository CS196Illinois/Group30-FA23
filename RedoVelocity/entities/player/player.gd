...
const FUEL_BURNING_SPEED = 10
const MAX_FUEL = 1000
const FEUL_REFUELING_SPEED = 20
		...
		states.GROUNDED:
			...
			if not is_grounded():
				if jump_escape_scenes < 0.1:
					velocity += gravitational_force * 1.6 * delta
				else: 
			...
			if current_planet.has_fuel: 
				if fuel < MAX_FUEL:
					fuel += FEUL_REFUELING_SPEED * delta
...
func jump(delta, check_on_ground, check_can_jump):
	...
	if check_on_ground:
		...
		if jump_escape_scenes > 0.1:
			...
			print("jump_escape_scenes: ", jump_escape_scenes)
